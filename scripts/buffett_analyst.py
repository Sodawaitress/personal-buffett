...
def _call_groq(system: str, user_msg: str, max_tokens: int = 300) -> str:
    ...
    # 429 重试策略：等满 65s（Groq 限流窗口 60s + 5s 余量），最多重试 2 次
    RATE_LIMIT_WAITS = [65, 65]
        ...
        try:
            ...
            if resp.status_code == 429:
                if attempt < len(RATE_LIMIT_WAITS):
                    ...
                    print(f"    ⏳ Groq 限流，等待 {wait}s 后重试（第{attempt+1}次）...")
                    time.sleep(wait)
                    continue
                else:
...
"""
    ...
    if trading_params and _trade_applicable and trading_params.get("entry_1_label"):
        ...
        if trading_params.get("entry_1_label"):
            parts.append(f"  建议买入区间1（MA60支撑）：{trading_params['entry_1_label']}")
        if trading_params.get("entry_2_label"):
            parts.append(f"  建议买入区间2（MA250年线）：{trading_params['entry_2_label']}")
