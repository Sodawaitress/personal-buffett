...
### US-202 续 · 那句「估值便宜」是怎么来的

补完数之后重查，发现根子不止一层。用户的问题「同一页面，一边说判断不了贵还是便宜，一边说好公司加便宜」，答案是**三层叠在一起**：

**第一层：字段从没补过**（已修，见上）。

**第二层：US-175 接错了地方。** 一次性收益识别 `has_tax_windfall` 当初只写进了 `_rate_legacy`，而线上真正跑的是 `rate_stock`。两个方法并存，我改了不跑的那个。所以 US-175 上线之后一直在写假的「便宜」——**代码写了、测试绿了、线上没生效**。

**第三层，也是最该记住的一层：措辞没跟着证据走。** 估值档有三条证据路径，优先级是 ① 跟同行比 ② 跟自己历史比 ③ 股价在 52 周区间的位置。三条路走完，结论一律被写成「估值X」。

但 ③ 只知道「股价比自己过去低」。**那不是估值。** 一只跌了 80% 的股票，如果利润跌了 90%，其实更贵了。把「跌得深」讲成「便宜」，是这个仓库反复犯的同一个错——**把受限的观测讲成不受限的结论**（US-151/163/176/182 同族），这次栽在了整个页面最显眼的那句话上。

**做法**：
- `_value_tier` 多返回一个 `basis`（`peer` / `pct` / `price`），措辞由 `_value_phrase` 按证据类型生成：靠分位才说「估值便宜（比自己历史）」，只有股价位置就说「股价处于偏低位置（估值本身判断不了）」
- PE 分位路径上，检测到一次性收益就不许判「便宜」，档位压到「偏贵」，理由直说「这个便宜不算数」
- `rate_stock` 补上 `has_tax_windfall`，并加一条**守卫接线而不是守卫算法**的测试：`rate_stock` 的源码里必须出现它

**顺带**：`_value_phrase` 原本收的是 `tier`（已本地化的中文串），英文句子会拼出 `valuation 便宜 vs peers`——小刚用的正是英文版。改成收 `rank`，措辞在各自语言里各自生成。

**这轮的方法**：三个 bug 都不是算错了数，是**话说得比证据大**。查这类问题的手法是固定的——把结论和它依据的那个字段并排摆出来，问一句「这个字段真的支撑得起这句话吗」。补数那条同样：不要问「代码写了吗」，要问「线上这个字段现在有值吗」。


### US-202 再续 · 改了说法，没改判断

第一轮修完，摘要已经诚实地写「股价处于偏低位置（估值本身判断不了）」。**但同一句话的尾巴还是「好公司 + 便宜，难得」，评级还是 A 级买入。**

因为措辞和打分是两个消费者。我修的是措辞那个，评级矩阵仍然拿 `rank=0` 当「便宜」。

> **改了说法不等于改了判断。** 一个结论修没修好，要看它下游的**每一个**消费者，不能只看最显眼的那句话。

**修法**：区分两种「只有股价位置」——

| profile | 含义 | 处理 |
|---|---|---|
| `v="price"` | 未盈利公司，本来就没有市盈率，股价位置是设计上唯一的估值轴 | 保留，正常参与评级 |
| `v="pe_or_price"` 降级 | 公司**有利润**，只是缺分位数据才掉到股价位置 = 我们不知道贵贱 | 估值轴按「无数据」中性处理 |

多邻国是后者，现在落到 `_combine` 里本来就存在的那条诚实分支：**B+ / 持有**，摘要写「好公司，价格合理（价格数据不足，先不下买入结论）」。

**这轮最该记住的不是修法，是守卫怎么两次假绿**：

1. 第一版守卫自己算了 `has_value` 再断言，没走真实路径 —— 把 `_value_tier` 的修复代码删掉，测试照样绿。**测试重算了被测对象的逻辑，就等于在考自己。**
2. 改成端到端调 `rate_stock` 之后还是绿 —— 因为测试数据缺 `net_margin`/`roe`，`rate_stock` 提前返回 `NR`，**根本没走到估值那段**。

第二种更阴：它长得完全正常，只是什么都没测。所以守卫里加了一条前置断言 `assert out["grade"] != "NR"` —— **先证明这条测试真的到达了它要守的地方**。

这和 US-174 是同一个坑：那次我的守卫测试有着和 bug 一模一样的缺陷，于是在坏代码上保持绿色。**判断一条守卫有没有用，唯一的办法是把修复删掉看它红不红。**


### US-202 收尾 · 两个市场是镜像的

用户问「改完是不是可以都重跑，A 股和美股有什么不同」。量了一遍生产，差别比预想的整齐：
...
- **美股**：能查一次性收益（有税前利润），但**没有任何估值分位** → 估值档只能靠股价位置 → 修完之后 38 只全部诚实地说「估值数据不足」
- **A股**：有分位能判贵贱，但**查不出一次性收益** → US-172 那套在 A 股上**一次都没生效过**

补数脚本 `backfill_tax_fields.py` 当初只写了美股那半边——名字里就带着 `US-172：给已有的**美股**年报补上`。

**A 股这半边挖下去是三层叠的，每层都足以让检测静默失效：**

1. **字段没存**。「利润总额」其实一直在抓（`fetch_cn_advanced` 里用来算 EBIT），只是没写进年报字段。加两行就有了。
2. **字符串解析不了**。A 股字段是 `'823.20亿'` 这种带单位的串，`float()` 抛异常 → `_num` 返回 None → `has_tax_windfall` 当成缺数据 → **永远返回 False**。它不报错，只是永远说「没有一次性收益」。
3. **单位不一致**。第一版我把税前存成「亿为单位的裸数字」`1147.55`，而同一行的 `net_profit` 是 `'823.20亿'` → 解析成 823 亿。两边差 1e8，**茅台被判成有一次性收益**（实测确实会）。

第 2 条和 US-154/168/171（本地 SQLite vs 生产 Postgres）、US-198（美股 Form 4 规则套到 A 股）同族：**拿一个语境的事实用到另一个语境**。这次形态最隐蔽——不报错，只沉默。

**守卫怎么写的**：
- 造一个**真有一次性收益**的 A 股样本断言为 True。茅台返回 False 可能是算对了，也可能是解析失败默认 False——不造正样本，前一条测试就是空转的绿。
- 单位守卫不在源码里找「亿」（它出现在转换器定义里，位置不稳），而是**把转换器抠出来真的调一次**，让单位自己说话。

**还剩一个真缺口**：美股没有 PE/PB 历史分位，所以 38 只美股现在集体是「估值数据不足 → B+/持有」。这是诚实的，但没用。补齐要么算美股自己的 5 年分位，要么建美股行业基准——是新活，没在这轮做。


## US-203 · 美股的估值分位：算得出来的算，算不出来的承认

> **实现状态（2026-08-30）**：✅ 已发布。已挂进 backfill-svc，随流水线每日补数。

US-202 收尾时留了个真缺口：**38 只美股一个估值分位都没有**，于是估值档只能靠股价位置，修完之后它们集体是「估值数据不足 → B+/持有」。诚实，但没用——等于对美股关掉了估值这根轴。

**先摸数据边界，再决定能做什么。** 实测 yfinance：AAPL / INTC / LULU / DUOL 一律只给 **4 期年度 EPS**、5 期季度 EPS。所以——

- **能做**：用「年度 EPS 阶梯」把周线价格换算成 PE 序列，算分位。实际窗口 3.5–4 年。
- **不能做**：真正的 5 年 TTM PE 曲线（缺季度历史）。

于是返回值里带 `years`，人话描述写「近 3.9 年的第 99 百分位」，**不许统一叫「5年分位」**。数据库那一列名叫 `pe_percentile_5y`——那是 A 股的语境（百度给的就是近 5 年）。同一列装两种窗口而不记录是哪种，就是这两天一直在修的那个毛病，所以加了 `pe_pct_window_years` 和 `pe_pct_range`。

**真实结果**：

| | 结论 |
|---|---|
| AAPL | 近 3.9 年第 **99** 百分位（区间 20.8–44.7 倍） |
| LULU | 近 3.5 年第 **6** 百分位（区间 8.4–76.5 倍） |
| NVDA | 近 3.5 年第 **21** 百分位（区间 32–380 倍） |
| INTC / ETSY / DUOL | 正利润历史不足 3 年，**没有可比的估值区间** |

最后一行是重点：多邻国 2023 年才首次盈利，INTC 刚扭亏。对它们来说**「没有可比历史」是正确答案，不是缺陷**。宁可空着。

**还原要先于排名**：被一次性收益抬高的那年 EPS 虚高 → 那年的 PE **假性偏低** → 它会成为历史区间的下沿，把当前值的分位往「贵」的方向拽。所以 `pe_percentile()` 接受 `normalized_eps_ratio`，先还原再排名。

**两个自己踩的坑，都写进守卫了**：
1. 排序之后取 `pes[-1]` 当「当前 PE」——那是**最大值**，于是每只股票都报第 100 百分位。AAPL 和 LULU 一起报 100，形态太整齐才看出来。
2. 一次性收益**压低** PE，不是抬高。我第一版的守卫断言了区间上沿会变，方向反了。

还有一个**测试替身**的教训：`_FakeHist` 少了 `.empty` 属性，被测代码把它当成空表提前返回 `{}`，测试红得像是算法坏了。替身得长得像真东西，缺一个属性就测了个寂寞。


### US-203 续 · 「正利润」不等于「有意义的利润」

生产首跑之后看结果，两件事：

**① 12 只算出分位全部写入失败**：`pe_pct_window_years` 列不存在。补数脚本直连数据库，**不经过 Flask 启动流程**，迁移不会自动跑。日志只报 `UndefinedColumn`，看着像权限问题。修法和 `scripts/industry_benchmarks.py` 一样——脚本自己先 `_migrate()`。

顺带把 `_is_us` 改名成 `_needs_percentile`：它的判据是「不是 6 位数字」，于是 `2359.HK` / `XRO.NZ` 也被放了进来。它们**确实**该被处理，但函数名说的是另一回事——名字和判据不一致，下一个人会照名字理解代码。

**② 更重要的：分位本身是错的，而且错得有方向。**

结果里有这么几行：

```
AIA.NZ  第 32 百分位（区间 28.4–2315.2 倍，中位数 259.1 倍）
LITE    第 46 百分位（区间 15.8–2623.5 倍，中位数 288.3 倍）
NVDA    第 0 百分位     FPH.NZ 第 1     HBM 第 1
```

2315 倍市盈率意味着那年 EPS 接近于零。我剔了负利润，**没剔接近零的正利润**——它是正数，`eps > 0` 就放它过去了。

后果**有方向，而且是最危险的那个方向**：任何从微利恢复到正常盈利的公司，历史区间被那段天价 PE 占满，于是今天无论多贵都排在最低分位。看着「史上最便宜」。
...
| SMCI | 第 12 | 第 55 |
| ANET | 第 68 | 第 **99** |
| AIA.NZ / LITE | 有分位（中位数 259/288 倍） | **没有可比历史** |
| **AAPL / LULU** | 99 / 6 | **99 / 6（纹丝不动）** |

最后一行是这条修改的**边界证明**：AAPL 和 LULU 没有微利年份，一个点都没被剔。修的是该修的那部分，没有误伤。

**判据**：`PE > 150` 视为「没有可比的利润」而不是「贵」；剔掉超过 25% 就整个返回空——**留一个勉强算得出的分位，比留空更有害**。

这仍然是 US-202 的同一个错误族：**把受限的观测讲成不受限的结论**。「微利年的 PE」根本不是估值信息。

**测试数据也踩了同一个坑**：三条已有的守卫在加了过滤之后集体变红——因为我原来的假数据用 EPS 1.0 配价格 200，PE 正好 200，被当成微利年剔了。**测试里的数字也得是现实里可能出现的数字。**


### US-203 三续 · 两个只有跑到生产才会暴露的错

**① 补数只补空值 → 算法修好了，坏数据留在库里。**

微利年过滤上线后重跑，`total` 从 38 掉到 26 —— 那 12 只带着修复前的错误分位（NVDA 第 0 百分位）安安静静留着，**日志还显示成功**。

「只补空值」是个隐含假设：**已有的值都是对的**。改了算法就不成立了。加了 `--refresh`。

**② 排名口径不一致 —— 又是让东西看起来便宜的那个方向。**

重算之后区间修正了（NVDA 从 32–379.6 变成 32–147.5），**但分位纹丝不动，还是第 0**。

原因：历史序列用的是**上一个完整财年的 EPS**，而生产传进去的 `pe_current` 是 **TTM（最近四个季度）**。成长股 TTM 利润更高 → TTM 市盈率更低 → 拿去跟按年报算的历史比，永远排在低分位。
...
三只全部朝同一个方向偏。这是 US-202 全篇的同一个错误族：**拿一个语境的数字放进另一个语境的排名**。

修法是把 `current_pe` 参数**删掉**，不是改默认值 —— **能传错的接口，迟早会被传错**。返回值里加 `current` 和 `basis`，人话描述写明「按上一完整财年利润计」。

**这两条的共同点**：本地测试全绿，逻辑也对，错的是「这个数从哪来、和谁比」。这类问题**只有真的跑一遍生产、并且盯着结果的形态看**才会露出来 —— NVDA/FPH/HBM 一起报第 0、第 1，形态太整齐，才让人起疑。

> 数字合理不代表对。**一组数字的形态**比单个数字更能告诉你哪里错了。


## US-204 · 自己用一次，第一位就是错的

> 用户：「我们这个网站做了这么久，结果我们自己不选几个玩玩，这不是知行不合一吗，也不方便我验证网站是否有用。」

**实现状态（2026-08-30）**：✅ 已发布。

加了 `audit_investable_ranking` 探针：把**我们真的买得到的**股票（Sharesies 支持 NZ/AU/US/HK，A 股 222 只买不到，排除）按网站自己的评级排出来。

跑出来第一位是 **T1 Energy（TE）· A 级 · 买入**，而它连市盈率都没有。

原因：它是未盈利太阳能公司，profile 走 `v="price"`，股价在 52 周区间第 8% → 估值档判 0（便宜）→ 质量 hi × 估值 0 → A 级买入。摘要一句话里自相矛盾：

> 「股价处于偏低位置（**估值本身判断不了**）。好公司 + **便宜**，难得。」

**US-202 修的时候我故意放过了这个分支。** 当时写的理由是：「未盈利公司本来就没有市盈率，股价位置是它设计上唯一的估值轴，把它也判成无数据等于废掉整个轴。」

那个理由是错的。**没有市盈率，不等于股价位置就变成了估值。它对谁都不是估值。** 我给一个已知有问题的分支找了个听起来合理的豁免,而它恰好是最容易出错的那类公司（未盈利、波动大、跌得深）。

修完：TE 从 **A/买入** → **B+/持有**，摘要「估值数据不足，只知道股价处于偏低位置。好公司，价格合理（价格数据不足，先不下买入结论）」。位置信息保留 —— 不驱动评级，但也别把已知的一起藏起来。

**这条的意义不在修法，在于它是怎么被发现的**：不是 code review，不是测试，是**第一次真的拿它给自己选股票**。

> 一个自己不用的工具，这种错会永远排在第一位，没人看见。

顺带记一条清单上剩下的可疑项：**MRVL 市盈率 106 倍，评级 B+**。因为它没有分位，估值轴中性化了，于是质量分单独决定了评级。「不知道贵贱」和「贵贱不重要」是两回事 —— 中性化解决了前者，但顺手制造了后者。待办。


## US-205 · A 股税前利润：写进了抓取，但抓取每轮只跑 2 只

**实现状态（2026-08-30）**：✅ 已发布。

US-202 把「利润总额」写进了 `fetch_cn_advanced`，本地验证通过（茅台 1147.55亿）。但全链条重跑之后一量：**A 股有税前利润的仍然是 0%**。

查下去：那一步在日常抓取里**每轮只跑 2 只** —— 它要打 3 个新浪报表接口，很重。222 只 A 股轮一遍要一百多天。

这和 US-202 开篇是同一句话，只是换了个形态：

> **需要慢慢等才生效的东西，等于没做。**

US-202 那次是「需要人手动跑一次」，这次是「需要等一百天」。两种都不叫落地。

照美股 `backfill_us_pe_percentile` 的样子补了 `backfill_cn_pretax`，挂进同一个 backfill-svc，每轮 30 只，几天补完。

**记一个检查动作**：改完一个字段之后，不要只验证「代码写对了」，要去量**生产上这个字段现在有多少比例有值**，以及**按当前速率补完要多久**。第二个问题我这次差点又没问。


### US-204 续 · 拿它给自己选股票，又找出两个

把评级依据（类型 / 年报年份 / 理由全文）打进探针之后，清单前两名各自暴露一个问题。

**① T1 Energy：「营收 2 年复合增速 25067%」→ 质量 100/100 → A 级买入。**

基数接近零算出来的复合增速。**和微利年的天价市盈率完全同形**：分母接近零 → 数字爆炸 → 被当成「特别好」。US-203 修了市盈率那个，这个漏了。

从 0.4 亿涨到 100 亿，和从 100 亿涨到 130 亿——前者百分比大得多，但它说明的是**「以前几乎没有生意」**，不是「现在增长更强」。

判据：复合增速 > 200%（三年翻 27 倍）视为「基数太小，增速不可比」，给 60 分并如实说明，不给满分。

**② Lululemon：A 级买入，依据是「ROE 31.8%（巴菲特线 15%）」+ 市盈率近 3.5 年第 6 分位。**

两个数都是真的。但它们描述的是**过去**。同期真实发生的：

| 财年 | 营收 | 毛利率 | 净利 | 增速 |
|---|---|---|---|---|
| 2024-01 | 9,619 | 58.3% | 1,550 | +18.6% |
| 2025-01 | 10,588 | 59.2% | 1,815 | +10.1% |
| 2026-01 | 11,103 | 56.6% | **1,579** | **+4.9%** |

增速连续腰斩、毛利率掉 2.6 个点、净利润绝对额下降。**9.4 倍市盈率配 4.9% 的增速，市场给的估值是对的，不是捡漏。**

「便宜」和「便宜是有原因的」必须能被区分开——这本来就是 `describe_cheapness` 那一栏要回答的问题，但它没出现在评级理由里。

加了 `_deterioration_flags`，三条独立信号（缺字段就跳过，不猜）：
- 增长减速：最近一年增速 < 长期复合的一半
- 净利润绝对额下降
- 毛利率同比掉 2 个点以上

LULU 三条全中；多邻国（增速 44→41→39%，利润和毛利率都在涨）一条都没有。**这个对照就是它没有乱插红旗的证明。**

> **ROE 描述过去。买入前要问的是现在正在发生什么。**

**这一节的方法**：两个错都是「拿它给自己选一次股票」找出来的，不是 code review，不是测试。而且找的手法一样——**把结论和它依据的那个数并排看**：「A 级买入」旁边写着「25067%」，「好公司+便宜」旁边是一条在下滑的利润线。


## US-206 · 每日五选的延迟：Gate 没坏，上游 cron 漂了

> 用户：「我发现每日五条的时间似乎在数据真的更新完前面，就导致每日五条的延迟。」

**实现状态（2026-09-02）**：✅ 已发布。

对的，而且比想的严重。

**routine 的时间极稳定**（UTC 13:16–13:37，从提交痕迹倒推，几乎同一分钟）。**漂的是流水线**：

| 日期 | schedule 实际启动 | 迟到 | 结束 |
|---|---|---|---|
| 08-25 | 07:41 | +18 分 | 10:19 |
| 08-26 | 07:43 | +20 分 | 11:42 |
| **08-27** | **18:04** | **+641 分** | 22:00 |
| **08-28** | **19:13** | **+711 分** | 22:40 |
| **08-31** | **14:49** | **+447 分** | 18:45 |
| **09-01** | **12:20** | **+297 分** | 16:43 |

`createdAt` 就是 18:04 —— **不是排队，是 schedule 事件本身晚了 11 小时**。

**6 次里 4 次，流水线结束时间晚于 routine 的触发时间**。那几天的五选读的必然是前一天的快照。迟到中位数 447 分钟。

**Gate 一直是对的。** routine 的提交信息里早就写着「pipeline 相位错位第 2 次」「双Gate FAIL 服务器数据未刷新」。Gate ① 的 26h 宽通道是按「结构性间隔 22.2h」设计的（US-149）—— cron 一漂，这个假设就不成立，Gate 诚实地报告了上游没跑完。

**问题在于处理方式：跳过。** 8 月跑 8 天跳 11 天（58%），同一个原因连着犯了一个月。

> **跳过不是处理，是记录。**

**三处改：**

**① cron 从 `0 7` 改成 `23 7`。** 整点是 GitHub Actions 上最拥挤的 cron 槽位，官方文档明确建议避开。07:23 启动 → 11:2x 出数据 → routine 13:2x 读，留 2 小时余量。

**② Gate ① FAIL 时先修再跳过。** routine 文档加了具体动作：查上游今天有没有触发过、失败或没跑就 `gh workflow run pipeline.yml` 拉起来、把修复动作写进说明。触发之后不等（流水线 4 小时，routine 等不起），今天照常按陈旧数据处理并标注基准日，**但明天的数据是新的**。静默跳过 → 一次自愈。

判断这一步有没有被执行：**连续两天 Gate FAIL 且都没触发过修复**，那是 routine 自己的问题。

**③ `scripts/cron_punctuality.py`。** 改 cron 时间是个赌注，不量就不知道赌赢没有。脚本直接报「结束时间晚于 13:20 UTC 的有几次」—— 那个数就是**五选读旧数据的天数**。
...
> 用户妈妈：「她上个月挣了点，突然战争了，又被吃进去了。」

**实现状态（2026-09-07）**：✅ 已发布。

查下来没有战争打到中国 —— 是中东局势推高油价（布油破 96 美元）+ 全球债市抛售。中国这边没事，是外围的钱在收缩。
...
而上证指数 8 月涨 4.0%、9 月只跌 1.1%，现在还比 7 月底高 —— **她的体感损失大于市场的实际损失**，差额来自她选的股票类型。

**国内软件为什么帮不了她**：同花顺/东财/雪球都只有「振幅」（当日最高最低之差），回答「今天活跃吗」，**不回答「平时颠成什么样、比大盘颠多少」**。

**能预测什么**：波动率聚集 —— 价格变化本身几乎没有自相关（方向不可预测），但**变化的绝对值有很强的长期自相关**。所以这个卡片的价值是**预期管理，不是择时**。

**措辞参考** Simply Wall St 的 "Volatility Over Time" 三段式（绝对水平 / 这个颠法稳不稳定 / 和别人比），但把打头的换成「**是大盘的几倍**」——「高于 92% 的 A 股」没有体感，「颠 3.9 倍」有。

实测：寒武纪 4.2× · 中际旭创 3.9× · 茅台 1.5× · 工商银行 1.0× · 长江电力 0.7×。

**三个自己踩的坑：**

1. **`stable` 写成了两态。** `if p.get("stable") else` 把 None（历史不够，判不了）当成 False，实测四只股票**全部**报「脾气最近变了」—— 一个从不说「稳定」的稳定性判断。又是把「不知道」讲成「知道」。改成三态。
2. **分位措辞反人性。** 给最稳的那只说「比 20% 的股票更颠」，读的人得在脑子里反算一次。低于 50 分位就说「更稳」。另外封在 1–99：「比 100% 的股票更颠」像口号不像事实。
3. **模板切片第四次切坏。** 按注释文字切，切在 `{# ... #}` 中间，报 TemplateSyntaxError，看着像模板坏了，其实是**测试的取法坏了**。这次写了 `_extract_block()` 按 `{% if %}`/`{% endif %}` 配对计数，一次写对，以后所有卡片渲染测试都能用。

**还有一次误报要如实记**：中际旭创和寒武纪在 260 天窗口下波动率都是 11.2%，我按「形态太整齐」的直觉判定是 bug —— 拉到 500 天分开了（10.4% vs 11.3%），**那是真巧合**。这个直觉这周救过四次，这次错了一次。
...
US-208 做的是单只。但用户妈妈的体感是**组合层**的 ——「我这个月白干了」说的从来不是某一只。所以自选股页顶部加一行。

**最容易写错的一条：组合波动 ≠ 个股波动的平均。**

五只都颠 4 倍的股票凑一起，组合不一定颠 4 倍 —— 它们不同涨同跌的那部分会互相抵消。直接平均会**系统性高估**，而且高估多少取决于相关性，不是一个固定折扣。

正确做法只有一个：**先合成组合每周的收益，再算这条序列的标准差。** 相关性就自动包含在里面了。

实测（生产数据）：

| 组合 | 实际 | 直接平均（错） | 分散省下 |
|---|---|---|---|
| 全高波动股（AI/电池） | **2.6 倍** | 3.5 倍 | 26% |
| 混合（含银行、电力） | **1.0 倍** | 1.6 倍 | **38%** |

**混合组的分散收益反而更大**（38% vs 26%）—— 因为银行电力和 AI 股不同涨同跌。这条直接推出页面上那句话：

> **分散本身就是降波动的手段，不需要专门去挑更稳的股票。多拿几只不相关的，比换成一只更稳的更有效。**

**页面上同时显示错误答案**（「直接平均会算出 3.5 倍，高估了」）。不是炫技 —— 是为了让人知道分散**省下了多少**，而不是以为组合本来就该这么稳。

**实现细节**：周收益序列存进 `vol_series`（JSON，约 100 个浮点 ≈ 1KB/只），组合计算才能实时做。基准序列缓存 6 小时 —— 每开一次自选股页就去拉 500 天日线太慢。

`user_watchlist` 只有 `buy_price` 没有持仓量，所以按**等权重**，并在页面上写明假设。

**守卫里有一条反向边界**：成员完全同涨同跌时，分散收益必须接近 0。如果那种情况也报出一个漂亮的折扣，说明算法在**凭空造分散**。


## US-210 · 周一体检：cron 那条没修好，而且它顺手破坏了 watchdog

**实现状态（2026-09-07）**：✅ 已发布。

### ① US-206 的 cron 改动**失败了**，而且我改错了方向

改成 `23 7` 之后连跑三天：

```
09-02  12:08  迟到 285 分
09-03  12:07  迟到 285 分
09-04  12:09  迟到 286 分
```

**四天全落在 12:07–12:20**。随机排队延迟不会这么整齐。

决定性证据：**cron 往后挪了 23 分钟，触发时间没有跟着动**（09-01 旧 cron→12:20，09-02~04 新 cron→12:08）。

> **GitHub 是在一个固定时段放行这个仓的定时任务，和 cron 写几点几乎无关。调分钟数是没用的。**

「避开整点」这条建议本身没错，但它假设延迟是**相对 cron 时间**的。这里不是。**我拿了一条通用建议，没有先验证它的前提在这个环境里成立。**

### ② 但 Routine 靠 US-206 第②条活下来了

它每天照跑，头一行自己写着：

> `📅 本文基于 2026-09-03（周三）收盘数据 · 09-04 pipeline 尚在跑 · Gate 全 PASS（age 20.73h）`

标注基准日、说明上游还在跑、照常出五选 —— 不再静默跳过。**结构性一天延迟仍在，但断供没了。**

### ③ cron 漂移顺手把 watchdog 变成了破坏者

watchdog 的判据只有「今天的快照 commit 了吗」，注释里的前提是「链条正常时 digest 约 11:36 UTC 就完成了」。**那个前提没了。**

于是它每天看到「今天没快照」→ 判定断链 → 补跑 digest，而 pipeline 自己的 digest 正在排队：

| 日期 | 快照生成次数 |
|---|---|
| 08-28 | **2 次** |
| 09-02 | **2 次** |
| 09-04 | **2 次**（且 pipeline 的 digest 被判 cancelled）|

**这正是 US-150「双跑 digest」那个 bug 从另一扇门回来。** US-150 的教训写的是「别加 cron」，但双跑其实**不需要新 cron —— 一个判据过时的兜底任务就够了**。

**修法**沿用 US-149 给 Routine 定的那条区分：**先看上游是不是还在动，再判它是不是挂了。** watchdog 现在先查 pipeline 有没有 `in_progress`/`queued` 的运行，有就不补跑、也不告警。

告警也挂同样的条件 —— 否则每天推一条假的「链条断了」，**真断的那天就没人信了**。

### ④ 其余各项健康

- **US-207 DDL 死锁**：修复后 09-03 全绿，是死锁修复以来第一次干净跑完
- **Routine**：09-01~04 每天都跑，Step 3.5 数据对刀在工作（五选里有「外网核实 H1 归母净利 -5.35%…便宜是有原因的短期承压，不是趋势坏」这种交叉验证）

### 留给下次

**cron 这条要换思路，不是换参数。** 可选：让 Fly 那个 24 小时在跑的应用去 dispatch pipeline，或者把流水线本身压短（现在 4.5 小时，其中 fetch 53m + radar 59m + DailyCron 75m）。**在此之前，「五选基于 T-1 收盘」是已知且已标注的状态，不是故障。**


## US-211 · 快照每天都在，但 digest 每天都被杀

**实现状态（2026-09-09）**：✅ 已发布。

周三体检：09-04 / 09-07 / 09-08 连续三天 pipeline 的 `digest` 判为 `cancelled`，**其余七棒全部 success**。
...
> **从产物上完全看不出来。** 快照每天都在，只是台账回填从没跑完过。

这类故障最难发现：**它的失败发生在成功之后。**

**根因**在 `pick_ledger.backfill()`：它把所有 `resolved_20d = 0` 的行全捞出来重跑，而**结不掉的行永远留在集合里** —— 某只股票缺价格数据 → `all_done` 恒为 False → 每天重查一遍，直到永远。

09-08 的日志里还在处理 **2026-07-05** 的推荐 —— 65 天前，早该在 20 个交易日内结清。这些死行把 digest 从 13 分钟拖到 21 分钟。

**三处改：**

1. **封存死行**：超过 45 个日历天（≈20 个交易日 + 宽限）还没结清的，就地标记 resolved。到那个点还补不上的数据，再等也不会有。
2. **限量 + 最老优先**：`backfill(limit=…)`，`ORDER BY pick_date ASC`。限量不排序等于「永远只做前 N 个」，进度不单调。
3. **拆开快慢**：落账（几秒）留在 digest，回填搬去 backfill-svc 并行跑。**落账要快，回填可以慢** —— 这两件事被绑在一起，慢的那件拖垮了快的那件。
...
|---|---|
| 09-04 | 2 次 ⚠️ |
| 09-07 | 2 次 ⚠️（修复 10:48 部署，但当天 pipeline 13:32 才起跑，watchdog 检查时它确实不在途 —— 这次补跑是**对的**）|
| **09-08** | **1 次 ✅** |
...
09-08 启动 12:10（迟到 287 分），09-07 启动 13:32。**「五选基于 T-1 收盘」仍是已知且已标注的状态。** 换思路的两个方向（Fly 触发 / 压短流水线）还没动。


## US-212 · 专栏《什么时候该卖》

> 用户：「我们的文章那篇怎么持有怎么卖的是不是还没发出去」

**实现状态（2026-09-14）**：✅ 已发布。确实没写过 —— 之前只在对话里讲了，**从没落到网站上**。

「讲过」和「发出去」是两回事。对话里的内容不会进入任何人的阅读路径。

**这篇最该守的不是文笔，是三条对自己不利的诚实：**

**① 引用被质疑的数字时必须带上质疑。** Morningstar《Mind the Gap 2026》说投资者因择时不当每年少赚 1.2 个百分点，这个数字被广泛引用。但 CFA Institute 的《Financial Analysts Journal》2026 年登了一篇论文，**用同一份数据**算出只有 0.10% —— 小十倍。两个都写进去了。

**② A 股那一节是单独写的，不是把美股结论翻译一遍。** Odean「被过早卖掉的赢家继续跑赢」建立在美股有动量效应之上；中国市场的研究是**短期（1–6 个月）和长期（3 年）都显著反转，没有持久动量**。所以文章明说：<b>在 A 股不能拿「历史上赢家会继续赢」当理由劝人别卖，那条依据不在。</b>

而且成本差异会**反转结论**：A 股一进一出约 0.056%、个人免征资本利得税，「卖了再买等于花钱回到原地」这个论证在这里几乎不成立。结论改成：**在高费率市场是费用替你拦住冲动，在 A 股没有东西替你拦 —— 只能靠规则。**

**③ 承认「知道」治不好。** 去偏实验（*Frontiers in Behavioral Economics*, 2024）的结论：当场有效、两周减弱、**三个月后消失**，而且**收益没有变好**。这条对文章本身不利 —— 不写，它就变成一篇「读完就好了」的鸡汤。

守卫七条，全部守的是上面这些不许被简化掉的地方，外加 US-187 那条：**不许针对特定某个人**（站上不止一位用户，困惑本来就是普世的）。
...
```
/          HTTP 302 → /login      （正常，未登录跳转）
/login     HTTP 200  3418B        （正常）
登录接口   HTTP 200（无效凭据）    （正常重渲染，没有 500）
```

**站点是活的，登录流程也正常。** 所以是「登不进去」不是「网站挂了」，最可能是**网络层** —— `fly.dev` 从中国大陆访问一直不稳定。已请用户确认她看到的具体现象（超时／密码错／白屏），这三种的修法完全不同。

**这条记一下方法**：「进不去」是个**症状词**，它同时覆盖了网络、认证、应用三层完全不同的故障。在问清楚是哪一层之前做的任何修复都是猜。


## US-213 · CI 红了两周，我一次都没看

**实现状态（2026-09-14）**：✅ 已发布。

推完 US-212 顺手看了一眼 Actions，发现 **CI 是红的**。再往回翻 —— **最近 200 次运行里一次都没绿过。**

而我这两周每次推代码，都只跑本地、然后宣布「652 条全过」。**一次都没看过 CI。**

> **一个永远红的 CI 比没有 CI 更糟：它制造了有安全网的假象。**
> 红久了就没人看，看了也不信 —— 它从信号退化成了噪音。

**红的原因很朴素**：6 条测试读「用户 1 的自选股」。本地 `data/radar.db` 有真实数据所以过，CI 上是空库，`_codes()` 返回 `[]`，于是 `assert len(codes) >= 20` 必然失败。

**修法不是 skip。** skip 掉等于这 6 条在 CI 上从不执行 —— 和删掉没有区别，而且更隐蔽（报告里显示的是「skipped」不是「missing」）。**这一周反复栽的「空转的绿」就是这个形态。**

正确做法是 `tests/conftest.py` 里一个 session 级夹具：**只在库完全为空时**播 25 只最小数据集。本地有真实数据 → 什么都不做，行为和现在完全一样；CI 空库 → 播种，那 6 条**真的执行**。

**播种代码本身也会写错**：第一版 INSERT 写了 `users.name`，而真实 schema 是 `display_name` —— 结果 **652 条全部 error**。session 级 autouse fixture 一挂，整个套件就没了，而报出来的是一堆 SQLAlchemy 栈，看着像被测代码坏了。现在包了一层，失败时报人话并指明「多半是列名和 schema 对不上」。

**守卫三条，最有力的是第三条**：直接开一个临时空库把整套测试跑一遍。它就是 CI 的环境 —— 将来谁再加一条依赖本地数据的测试，这里会先红。

**这一条的教训不在代码**：

> **「我跑了测试」和「测试在该跑的地方跑过了」是两回事。**
> 这和 US-212 那句「讲过 ≠ 发出去」是同一个形状 —— 一件事在我这里完成了，不等于它在系统里完成了。


## US-214 · 每一项研究都要标出它来自哪个市场

> 用户看到文章第三节「交易越多，赚得越少」：「这个是美国的研究吧？有没有中国的 中美不同的研究要标注清楚」

**实现状态（2026-09-14）**：✅ 已发布。

问得对。一到四节**全是美股数据**（Odean 的 10,000 个账户、Barber & Odean 的 66,465 个家庭、去偏实验、Madrian & Shea），但读起来像普世结论 —— 只有第五节才提 A 股。

**这正是本站反复在修的那族错误：把一个语境的事实用到另一个语境。** 我在文章里专门写了这条，然后在同一篇文章的排版上犯了它。

**做法**：每个知识点标题带国别徽章 —— 🇺🇸 美国数据 / 🇨🇳 中国数据 / 🇺🇸🇨🇳 两边都测过。顶部加一句：**标 🇺🇸 的不要默认适用于 A 股。**

**顺带补上了中国的对应研究**（这条恰好两边都测过，而且中国的结论更狠）：

- 廖理、贺裴菲、张伟强、沈红波（2013）《中国个人投资者的过度自信和过度交易研究》，清华大学五道口金融学院
- 史永东、李竹薇、陈炜《中国证券投资者交易行为的实证研究》，**深交所全部投资者账户**

结论：换手率越高的散户投机偏好越重、业绩越差；**持股不足三个月的显著弱于市场平均**；小散户的净买入与**次日收益显著负相关** —— 今天买的明天多半跌。

守卫的写法值得记：**不是检查「有没有提到中国」，而是把所有知识点标题抓出来，逐个检查有没有国别徽章**，只允许纯解释性的（不含具体研究和数字）那两条例外。这样将来新增一节忘了标，测试会点名是哪一节。

### 顺带：登录页 Demo 入口藏起来

登录页太挤。这个站现在的用户全是认识的人，没有「先试用再注册」这个场景。

**藏不是删** —— `demo_login` 路由保留，直接访问仍可用；删路由会连带删掉一批依赖它的测试。要放回来设 `SHOW_DEMO_ENTRY=1`，不用改代码。
...
**实现状态（2026-09-14）**：✅ 机制已发布；文章本身将随下一次 routine 推送。

查下来答案是：**上线了，但没发出去。**

微信推送只在 `output/daily_push.txt` 变动时触发（`wechat-push.yml` 的 `paths:` 只有这一个），而 **daily_push 从来没提过文章**。

所以站上**7 篇专栏，一篇都没通知过读者** —— 全部写完、部署，然后躺着等人自己发现。

这和 US-212 那句「讲过 ≠ 发出去」是同一件事再深一层：

> **写完 ≠ 发布。发布 ≠ 送达。**
> 每一层之间都有一个会静默丢东西的缺口。

**修法不开第二条推送通道** —— US-150 的教训是并行通道会让读者收到重复推送。改成把「哪几篇还没通知过」**算好塞进快照**，沿用 US-166 那条已经写在仓里的判断：

> 在代码里算好塞进快照，而不是写在 CLAUDE_ROUTINE 里让 Routine 每天记得回头查 —— **本仓所有沉默失败都证明「靠自觉」不成立。**

**做法**：`snapshot["new_articles"]` + `knowledge/announced_articles.txt` 记录已通知的 slug。Routine 在 daily_push **末尾**加一行（五选是主线，文章是附带的，放前面会挤掉主线），提完调 `mark_announced()`。

**一次最多两篇**，攒了一堆不刷屏。**忘了 mark 会重复提 —— 烦，但看得见**，比静默漏掉好。

**旧的 5 篇做了补标**：它们其实从没通知过，但一次性推 7 篇会刷屏。所以只让最新一篇走推送，其余靠专栏页被发现。**这是一个取舍，不是记录事实** —— 所以写在 `announced_articles.txt` 的注释里，免得以后有人把它当成「通知过了」的证据。


## US-216 · 钱从哪里赚 —— 存数字，不存分类

> 用户妈妈：「有些评级 A 的票因为做外贸、受汇率影响导致没涨」

**实现状态（2026-09-20）**：✅ 已发布。

### 先说查出来的结论：不全是汇率

**汇率这件事本身是真的**：人民币一年升值 **6.0%**（近三个月 1.1%）。对出口商是直接逆风。

**但数据不支持「因此没涨」。** 用**真实的海外收入占比**（财报主营构成按地区）测 A 级股：

| 代码 | 名称 | 海外% | 12月超额 |
|---|---|---|---|
| 603338 | **浙江鼎力** | **83.5** | **+2.4** |
| 300274 | 阳光电源 | 73.4 | −37.9 |
| 300760 | 迈瑞医疗 | 53.4 | −32.6 |
| 300124 | 汇川技术 | 6.6 | −36.3 |
| 600415 | 小商品城 | 0.9 | −35.8 |

**相关系数 r = −0.18** —— 方向对，但几乎是噪音。海外占比最高的浙江鼎力是**唯一跑赢的**；跌最惨的汇川技术海外只占 6.6%。

更可能的解释：跌的那几只各有各的行业原因（光伏产能过剩、医疗集采、新能源车价格战），汇率是叠加项不是主因。汇川的情况 routine 9 月 4 日已查明：H1 归母净利 −5.35%（新能源汽车 + 汇兑双压），但**扣非仍 +3.13%、营收 +20.31%**。

### 我自己犯的错，也是这条 US 的起点

第一轮我**按行业知识分组**，得出「出口组跑输 15 个百分点」。去核实主营构成才发现：**我把汇川技术划进了出口型，而它 93.4% 是内销。**

**分组是我编的，结论就是假的。**

所以模块**不做分类**，只存一个可核验的数字：
...
需要标签时从数字派生，**阈值写在模块里（`_HIGH=40 / _MID=15`）—— 藏起来的阈值等于没有依据**。

### 三个设计决定

1. **同时存报告期**（`overseas_asof`）。一个过期的占比比没有更误导。
2. **识别不出的地区算作海外**。宁可高估敞口 —— **低估会让人以为不受影响**，那个方向更危险。
3. **卡片只说敞口，不说涨跌**。展开层里直接写明「r = −0.18，几乎是噪音，海外占比最高的那只反而是唯一跑赢的」。没有这句，读者会自然地把「海外占比高」读成「所以会跌」—— **那正是我犯过的错**。

### 一个值得单独记的坑

新浪利润表里有「**汇兑收益**」这一列，看起来正是最直接的指标（财报里真实发生的那笔钱）。实测 4 只公司 × 3 年，**值全是 NaN**。

> **列存在 ≠ 数据存在。**

如果没实际取一次值就用了它，会得到一个永远为空却看起来有来源的字段。

### 测试替身的正解

第一版我又造了一个 DataFrame 替身，又写崩了（US-208 同款）。这次的解法不是「把替身写对」，是**直接用真的 pandas**。

> 教训的正解往往不是把错的东西做对，是**别做那件事**。
...
对。我把「**我的分类是错的**」过度推成了「**不要分类**」。

> **分类给理解，数字给核验，缺任一个都不行。**
>
> - 只有分类 → 无法核验 = 我一开始犯的错（凭印象分组）
> - 只有数字 → 「83.5%」算多还是少？= 我矫过头的错
> - 分类 + 数字 → 「海外为主 · 汇率影响大 / 海外 83.5% · 国内 16.5% / 截至 2025-12-31 财报」

真正的规矩不是「不分类」，是 **分类必须从测量派生，不能从记忆派生**，而且阈值要写出来、可以被质疑。

**而且我确实做歪了**：`label()` 算出来了，**卡片上却没显示** —— 她只看得到一个数字。

**改了两处：**

1. **分类打头，数字支撑。** headline 从「海外收入 83.5%」改成「海外为主 · 汇率影响大」，数字下沉到 figure 行。
2. **阈值从 40/15 改成 50/15，措辞直接回答她问的那件事。** 第一版把 53.4% 的迈瑞标成「主要做外贸」—— 那其实是海外国内各一半，**标签比实际说得大**，同样是把受限的观测讲成不受限的结论。

三档，每一档直接对应「这只受不受汇率影响」：
...
**这次的形态值得单独记**：我从一个错误直接荡到了它的反面。
「分组是我编的」是对的诊断，「所以别分组」是错的处方 ——
**诊断正确不保证处方正确。**
...
**按订单实时汇率 —— 做不到。** 那是公司内部数据，财报不披露，公开最细只到半年度分地区收入。

**但这个问题指向了一个更好的答案：公司每半年自己报一次财务费用。**

| 公司 | 海外% | 2025H1 | 2026H1 | 增量占营收 |
|---|---|---|---|---|
| **浙江鼎力** | 83.5 | **−2.90亿** | **+1.66亿** | **8.42%** |
| 迈瑞医疗 | 53.4 | −3.36亿 | +3.83亿 | 4.05% |
| 阳光电源 | 73.4 | −2.63亿 | +3.68亿 | 2.04% |
| 汇川技术 | 6.6 | −0.52亿 | +0.49亿 | 0.41% |
| 贵州茅台 | 1.1 | −4.87亿 | −2.43亿 | 0.26% |

财务费用是负数 = **这一项在赚钱**（利息收入 + 汇兑收益 > 成本）。去年出口商都在赚，**今年全部翻成了净支出**，而且基本跟着海外占比走。茅台至今仍是负的。

### 报出来的数比估算准，差了快一倍

我估浙江鼎力「营收拖累约 4.5%」，**实际财务费用增量占营收 8.42%**。

因为估算只覆盖**营收换算**，而汇率还打在**应收账款和外币资产**上，那部分直接进损益。

> **能拿到报出来的数时，不要估。**

### 一条不能省的措辞

**财务费用 = 利息 + 汇兑 + 手续费，拆不开。** 单独的汇兑字段实测拿不到：

- 新浪利润表「汇兑收益」列 → **值全是 NaN**
- 东财 `EXCHANGE_INCOME` → **6 家里 1 家有值，且为 0.00 亿**

所以卡片只说「财务费用变化（含汇兑）」，**绝不说「汇兑损益是多少」**。守卫里有一条专门盯这个措辞。

### 同口径比较是硬要求

`finance_swing()` 按**同月同日**匹配上期（中报比中报、年报比年报）。拿 2026 中报去比 2025 年报，会把「半年 vs 全年」的差当成变化 —— **本仓那族错误的财务数据版本**。守卫里造了一个带年报干扰项的样本专门验这条。

汇率也用**期间平均对比**（2025H1 平均 724.32 → 2026H1 平均 685.44，升值 5.4%），不是两个时点的即期汇率 —— 营收换算的正确口径是平均。

### 顺带纠正我自己的一次过度断言

我在中途输出里写过「即期和期间平均差 0.5 个百分点，用错口径结论会偏」。**那句话是错的** —— 那两个数（12 个月即期 vs H1 对 H1 平均）测的不是同一个东西，拿它们相减去论证「口径重要」是拼凑证据。

口径这件事本身成立，**但我举的那个证据不成立。**


## US-218 · 汇率影响的三层

> 用户：「研究一下学术最新业界最优怎么从技术上解决这个问题 怎么从 UI 上最好的展示这个因果链在哪里」+「影响的层次」
...
| 层 | 是什么 | 时间 | 能算吗 |
|---|---|---|---|
| **① 交易敞口** transaction | 签约 → 收款之间汇率变了 | 数周~数月 | ❌ 财报不披露 |
| **② 折算敞口** translation | 合并报表时的账面换算 | 每个报告期 | ✅ 财务费用摆动 |
| **③ 经济敞口** economic | 人民币贵了，东西卖不动 | 数季度~数年 | ⚠️ 估算 |

**用户上一个问题「能不能按订单实时汇率算」问的正是 ① 层** —— 现在能说清为什么拿不到。而 ③ 层是**最大但最慢**的：它不进财务费用，它直接吃掉订单。

**恒定汇率（constant currency）是业界标准技法**：把本期按上期平均汇率重算，差额即汇率贡献。我们没有分币种原币收入，用一阶近似 `报告增速 + 海外占比 × 汇率变动`，**所以必须标成估算**。

实测：浙江鼎力报告 +24.9%，恒定汇率下约 **+29.4%** —— 汇率拿走约 4.5 个百分点，**但它即使这样还是涨了 24.9%**。阳光电源 −29.0% → −25.0%，**汇率解释不了它的下跌**。

还查到一条容易做错的：收入桥拆解要**先把两期都换算到恒定汇率，再算量价结构，残差才是汇率**。顺序反了，汇率会污染价格项。

### UI 侧：两个结论，一个和直觉相反

**① 不用瀑布图。** 它是业界分解因果的标准画法，但对新手有**内在**障碍：正值从下往上读、负值从上往下读；中间分项没有共同基准轴。128 人的研究里参与者可视化熟悉度平均 2.09/5，瀑布图正是难点。本站主要读者是年长非专业的手机用户 —— 用现有的「横条 + 文字」，不引入新视觉语言。

**② 估算层要更淡，不是更醒目。** 这条和我的直觉相反 —— 我原本打算把估算「单独标出来」，潜台词是让它更显眼。**方向错了。**

> 可视化研究的规则：**对一个估算越没把握，就让它在视觉上越不突出，这样更确定的数据才会得到更多注意。**

所以 `certainty` 是结构化字段，直接驱动 CSS：

```
measured    16px 粗体带色        ← 报出来的数
estimated   12px opacity .72     ← 估算，退后
unavailable opacity .5 斜体      ← 算不出，但**必须显示**
```

**③ 来源标记融进显示本身**（【报出来的数】/〔估算〕跟在每层旁边），不做脚注 ——「数据来源的可视化应当无缝融进看板」。

**① 层算不出也要显示**，因为**那一层的缺失本身就是答案** —— 它解释了用户问的「为什么不能按订单算」。省略掉，那个问题就永远悬着。

守卫里有一条直接读 CSS 比字号和 opacity：**报出来的数必须比估算更醒目**。反向验证过 —— 把估算层调大 → 立刻红。


## US-219 · 本期至今 —— 唯一不是事后分析的那一层

> 用户：「你这些都是事后分析了 已经涨了的 市场预期已经搞了不是吗」

**实现状态（2026-09-20）**：✅ 已发布。

**这个批评成立。** US-217/218 分解的财务费用来自 2026 中报，**8 月就公布了**，市场早消化了。

但有一段不是：**汇率每天可观测，而本期的账要等下一份财报才披露。**

```
2025 年 7–9 月平均 714.56  →  2026 年 7–9 月 673.41   升值 +5.8%
...
这一段卡在「**已经发生在账上**」和「**还没人报出来**」之间 —— 正是本仓五层传导链要找的那种东西。

### 视觉编码：位置 = 时间，强度 = 确定性

它排在**最前面**（最新），但**和第③层一样淡**（同样是估算）。

**两个维度用两种编码，互不干扰** —— 否则「最新」会被读成「最重要」，而它不预测收益（实测海外占比与 12 个月超额收益 r = −0.18）。

用户对措辞的要求是「**写上是估算就好**」—— 所以只有一个〔估算〕标签 + 一条左侧虚线边（表示还在累积、没结算），**没有堆警告**。

### 对齐同一段日历窗口

`_fx_pending()` 拿「今年 7 月 1 日 → 今天」比「去年 7 月 1 日 → 去年同一天」。

**拿「今年 7-9 月」去比「去年 7-12 月」会把季节性当成汇率变动** —— 本仓那族错误的又一个版本。守卫直接读源码检查窗口对齐。

### 用户的批评还指出了一件更大的事

**我这几轮做的功能全是「解释过去」，没有一个是前瞻的。** 而这个仓的核心架构（五层信息传导链）本来就是为了**找还没传到价格上的东西**。
...
> 用户：「已经发生的也是一个很好的归类 应该归到现在股价偏高不是吗」
> 「不讲 而是分区 一个区是消化掉的消息 一个区是没消化掉的 没消化掉的按两个方式排 影响程度 和确信度」

**实现状态（2026-09-20）**：✅ 已发布。

### 用户的两个判断都对，而且比我的方案好

**第一个**：汇率打掉利润 → 同样的股价 → **市盈率显得高**。这和多邻国那次是同一逻辑的反面：

| | 对利润 | 对市盈率 |
|---|---|---|
| 一次性退税（DUOL） | 虚增 | **假性偏低**（看起来便宜） |
| 汇率损失（出口商） | 压低 | **假性偏高**（看起来贵） |

浙江鼎力中报净利 9.90亿，财务费用恶化 4.56亿 → **利润被压低约 31.5%**，市盈率因此偏高约 46%。

**第二个，而且更重要**：我原本打算在估值卡旁边**加一句话解释**「它为什么看起来贵」—— 那还是把最重要的区分当成了脚注。

> **「市场看过没看过」应该是主轴。**
> 看过的那些，价值在于**解释现在的价格**；没看过的那些，才是还可能动价格的部分。

这直接回答了用户前一个批评（「你这些都是事后分析了，市场预期已经搞了」）—— 不是辩解，是把它变成信息架构。

### 「已消化」的判据必须可观测

我们无法知道市场**是否正确定价**了一条消息。能观测的只有**它有没有被公开**。

所以分区判据是「已公布 / 未公布」，文案写「**市场已经看过**」而不是「市场已经消化」—— 后者是对市场行为的断言，我们没有证据。卡片底部明写「『看过』指的是已经公布，不是市场一定定价对了」。
...
|---|---|
| **影响程度** | 占营收的百分比，可比 |
| **确信度** | 高=报出来的数 / 中=公开数据的算术 / 低=需要行为假设 |

**两个维度分开存**，因为排序方式由用户选，不由我们定。切换用纯链接（`?fxsort=impact` / `?fxsort=conf`），不需要 JS —— 和本站其余部分一致。

**「算不出」的那条要留着且排最后**（经济敞口），不能假装它影响为 0 —— 同 US-218 的原则：缺失本身是信息。
