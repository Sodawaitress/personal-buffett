...
### US-31 · 我的选股：搜索过滤 + 列表模式

**As a** 用户  
**I want to** 在选股页快速找到某只股票，并在股票多时用列表模式操作  
**So that** 持仓超过10只时不用上下翻卡片，一眼扫完，一键操作

**工具栏搜索框：**
- 实时过滤（输入即筛）：匹配股票名称或代码，不区分大小写
- 清除按钮（×）一键重置
- 搜索结果为空时显示"没有匹配的股票"
...
□  C     中金公司    CN    ¥16.80     +0.1%   观望   04-03      ↻ 读信 移除
```
- 每行高度紧凑（40px），无摘要文字
- 点击名称 → 详情页
- 勾选列 → 批量操作底部栏
...
**视图切换：** 工具栏 卡片 | 列表 | 缩略图（三选一，localStorage 持久化）

**Acceptance Criteria:**
- 搜索框实时过滤，≤ 16ms 无感延迟（纯客户端，不发请求）
- 列表模式所有操作（分析/读信/移除/勾选）均可用
- 搜索 + 排序 + 筛选 可组合使用（互不干扰）
- 移动端（< 768px）列表模式自动降级为卡片
